...
i = 19;
ephysStruct(i).neuronNumApproach = 1;
...
% LRClass = [zeros(size(wLProj{1},1),1); ones(size(wRProj{1},1),1); zeros(size(pLProj{1},1),1); ones(size(pRProj{1},1),1)]; 
...
% for i = 1:numDim
%     LRData{i} = [wLProj{i}; wRProj{i}; pLProj{i}; pRProj{i}];
%     mdlTable = array2table(LRData{i});
%     mdlTable.Strain = Strain;
%     mdlTable.LRClass = LRClass;
% 
%     mdl{i} = fitglm(mdlTable.Strain,LRData{i});
% end
