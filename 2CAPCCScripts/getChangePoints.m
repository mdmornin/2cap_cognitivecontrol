function changePoints = getChangePoints(trlStruct,figSavePath,masterTbl,flags)
    % Blank vars
    regAppCP = []; regCorrectionCP = [];
    revAppCP = []; revCorrectionCP = [];

    if strcmp(flags.SessionN,'all') && strcmp(flags.Genotype,'all')    
        regIdx = startsWith(masterTbl.SessionType,'Regular');
        revIdx = startsWith(masterTbl.SessionType,'Reversal');
    ...
    elseif strcmp(flags.SessionN,'all') && strcmp(flags.Genotype,'P')
        regIdx = startsWith(masterTbl.SessionType,'Regular') & strcmp(masterTbl.Strain,'P'); 
        revIdx = startsWith(masterTbl.SessionType,'Reversal') & strcmp(masterTbl.Strain,'P');
    ...
    elseif strcmp(flags.SessionN,'all') && strcmp(flags.Genotype,'W')
        regIdx = startsWith(masterTbl.SessionType,'Regular') & strcmp(masterTbl.Strain,'W');
        revIdx = startsWith(masterTbl.SessionType,'Reversal')& strcmp(masterTbl.Strain, 'W');
    ...
    % Output data
    changePoints.regApp = regAppCP;
    changePoints.revApp = revAppCP;
    changePoints.regCor = regCorrectionCP;
    changePoints.revCor = revCorrectionCP;
